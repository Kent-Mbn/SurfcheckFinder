...
-(void)setFeedData:(SPH_PARAM_List *)feed_data
{
    if ([feed_data.chat_media_type isEqualToString:kSTextByme])
    {
        ...
        textMessageBubble.frame = CGRectMake([Common widthScreen]- 50 -textMessageBubble.frame.size.width,0, textMessageBubble.frame.size.width, textMessageBubble.frame.size.height);
...
//        }
        ...
        UIImageView *AvatarView=[[UIImageView alloc]initWithFrame:CGRectMake([Common widthScreen] - 50, self.frame.size.height-50, 40, 40)];
        [AvatarView setImage:[UIImage imageNamed:@"person"]];
        
        AvatarView.layer.cornerRadius = cornerRadiusBubble;
        ...
        if ([feed_data.chat_media_type isEqualToString:kSTextByOther])
        {
            ...
            if ([feed_data.chat_media_type isEqualToString:kSImagebyme])
            {
                ...
                flowerImageBubbleView.frame = CGRectMake([Common widthScreen] - 90 - 50,0, 90, 90);
                
                [self.contentView addSubview:flowerImageBubbleView];
                
                
                UILabel *timeLabel=[[UILabel alloc]initWithFrame:CGRectMake(0, self.frame.size.height-90, 55, 20)];
                timeLabel.text=feed_data.chat_date_time;
                timeLabel.font=[UIFont systemFontOfSize:9];
                timeLabel.textColor=[UIColor blackColor];
                [self.contentView addSubview:timeLabel];
