...
-(void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations{
    ...
    NSLog(@"locationManager didUpdateLocations TRACKER");
    
    for(int i=0;i<locations.count;i++){
        CLLocation * newLocation = [locations objectAtIndex:i];
        CLLocationCoordinate2D theLocation = newLocation.coordinate;
        CLLocationAccuracy theAccuracy = newLocation.horizontalAccuracy;
        //NSLog(@"Location Update: %@ %f", newLocation, theAccuracy);
        /*
    ...
    self.shareModel.timer = [NSTimer scheduledTimerWithTimeInterval:TIME_TO_RESTART_CHECKING_LOCATION target:self
    ...
    self.shareModel.delay10Seconds = [NSTimer scheduledTimerWithTimeInterval:TIME_TO_RESTART_LOCATION target:self
...
-(void)stopLocationDelayBy10Seconds{
    ...
    NSLog(@"locationManager stop Updating after 60 seconds");
}
...
- (void)updateLocationToServer {
    ...
    if (CLLocationCoordinate2DIsValid(self.myLocation) && [[UserDefault user].token length] > 0) {
        //Lat Long of user not valid
        [self callWSUpdateLocation:self.myLocation];
    }
}
